...
UNIT_BANK = unit_bank(
    ...
    concept_overview='動的計画法は、小さい状態の答えを先に求め、その結果を使って大きい状態の答えを作る考え方です。状態・遷移・初期値を整理する基本を身につけます。 この unit では、既習の グリッドDP（経路数え上げ） の基本 と グリッドDP（経路数え上げ） を素直に実装する を順に使いながら、1 問を分けて解く流れまで確認します。',
    problem_bank=[
        problem(
            problem_id='algo-046-integration-p1',
            title='グリッドDP（経路数え上げ） の総合演習 / 1 ケースをそのまま解く',
        ...
        problem(
            problem_id='algo-046-integration-p2',
            title='グリッドDP（経路数え上げ） の総合演習 / 2 ケースを連続して解く',
        ...
        problem(
            problem_id='algo-046-integration-p3',
            title='グリッドDP（経路数え上げ） の総合演習 / 3 ケースを連続して解く',
