...
export function resolveOptionalHarnessAssetPath(
  ...
  const candidates = packageRelativePath.endsWith(".md")
    ? [packageRelativePath]
    : [packageRelativePath, `${packageRelativePath}.md`];

  for (const candidate of candidates) {
    const projectPath = options?.projectInHarnessDir === false
      ? join(projectRoot, candidate)
      : join(projectRoot, ".harness", candidate);
    if (existsSync(projectPath)) return projectPath;

    const packagePath = join(import.meta.dirname ?? "", "..", "..", candidate);
