...
import { resolveRequiredHarnessAssetPath } from "../../infrastructure/assets/review-assets.ts";
...
export function prepareComponentFlowRuntime(params: RuntimeParams): ComponentFlowRuntime {
  const root = params.boundary.getProjectRoot();
  const logger = new HarnessLogger(`component_${params.scope.replace(/\//g, "_")}`, {
  ...
  return {
    ...
    lintGuard,
    reviewOrchestrator,
    criteriaPath: resolveRequiredHarnessAssetPath(root, "review-criteria-component.md"),
