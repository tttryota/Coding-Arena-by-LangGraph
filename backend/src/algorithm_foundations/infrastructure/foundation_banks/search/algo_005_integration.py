...
UNIT_BANK = unit_bank(
    ...
    concept_overview='幅優先探索は、近い状態から順に広げていく解き方です。キューを使って層ごとに進むことで、最短手数や到達可否を素直に求めます。 この unit では、既習の 幅優先探索（BFS） の基本 と 幅優先探索（BFS） を素直に実装する を順に使いながら、1 問を分けて解く流れまで確認します。',
    problem_bank=[
        problem(
            problem_id='algo-005-integration-p1',
            title='幅優先探索（BFS） の総合演習 / 1 ケースをそのまま解く',
        ...
        problem(
            problem_id='algo-005-integration-p2',
            title='幅優先探索（BFS） の総合演習 / 2 ケースを連続して解く',
        ...
        problem(
            problem_id='algo-005-integration-p3',
            title='幅優先探索（BFS） の総合演習 / 3 ケースを連続して解く',
