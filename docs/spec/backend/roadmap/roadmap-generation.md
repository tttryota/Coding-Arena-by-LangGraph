...
ユーザーが選択したトピックに対して、LLM が学習ロードマップを一括生成する。生成される構造で許可する階層は `major`（大枠）→ `middle`（中枠）→ `detail`（具体）の 3 種だけとし、親子関係もこの順序に固定する。全枝が必ず 3 層に到達することまでは要求せず、`major.children=[]` と `middle.children=[]` は許容する。生成は非同期ジョブとして受け付け、完了後はジョブ状態取得 API で `roadmap_id` を確認し、既存 B5 の retrieval で生成結果を参照する。
...
- 各具体項目の確認ポイントは 3〜5 個に収めること。コードで表現できるポイントは知識 + 実践のセットで出題されるため、結果として 6〜10 問程度になる。超える場合は内容で分割し、タイトルに範囲を明示すること
- 各項目には `title` と `description`（何を学ぶかの説明）を含めること
...
- 大枠を基礎・応用の 2 段階に固定する理由: 全トピックで学習体験を統一する。「実践」は基礎・応用の中の実務的知識として包含する。何が基礎で何が応用かの判断は LLM に委ねる。
- トピック名のみを入力とする理由: LLM の学習データに主要 IT 分野の知識体系は含まれている。ユーザーのノート内容はマッピング時に使用し、ロードマップ生成には不要。
