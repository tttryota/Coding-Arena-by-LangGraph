...
取り込まれたノート群からシステムがトピック候補を提案し、ユーザーが選択するとLLMがコーディング実践ロードマップ（学習段階→実装カテゴリ→演習ユニット）を生成する。各演習ユニットは段階的なコーディング問題に分解され、座学と実践を通じて実装スキルを習得する。

**C. コーディングセッション（メイン体験）**

ロードマップの演習ユニットを選んでセッションを開始する。まず座学で概念を理解し、その後段階的なコーディング問題(rewrite→fill_blank→bug_fix→extend→implement)に取り組む。LLMがコードを評価し、フィードバックを返す。結果はロードマップの進捗に反映される。
