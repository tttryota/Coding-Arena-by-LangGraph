...
UNIT_BANK = unit_bank(
    ...
    concept_overview='文字列処理は、文字の並びを比較したり、部分文字列を見つけたり、規則性を前計算で利用したりする知識です。並びをそのまま走査して構造をつかむ基本を学びます。 この unit では、既習の 回文判定 の基本 と 回文判定 を素直に実装する を順に使いながら、1 問を分けて解く流れまで確認します。',
    problem_bank=[
        problem(
            problem_id='algo-078-integration-p1',
            title='回文判定 の総合演習 / 1 ケースをそのまま解く',
        ...
        problem(
            problem_id='algo-078-integration-p2',
            title='回文判定 の総合演習 / 2 ケースを連続して解く',
        ...
        problem(
            problem_id='algo-078-integration-p3',
            title='回文判定 の総合演習 / 3 ケースを連続して解く',
