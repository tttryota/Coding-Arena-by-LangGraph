...
UNIT_BANK = unit_bank(
    ...
    concept_overview='最大流・マッチングは、通せる量や組み合わせをグラフの辺として表し、制約つきでどれだけ流せるかを考える知識です。まずは source から sink への最大フローを直接求める基本形を学びます。',
    problem_bank=[
        problem(
            problem_id='algo-123-basic-p1',
            title='最大フロー（Ford-Fulkerson法） の基本 / 頂点 1 から頂点 N への最大フローを Ford-Fulkerson 法で求めよ',
